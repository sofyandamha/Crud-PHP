<?php 
$query = "SELECT `mahasiswa`.`nama`,`matakuliah`.`nama_matkul`,`nilai`.`nilai`,`nilai`.`keterangan`,`matakuliah`.`id` FROM `mahasiswa`
	INNER JOIN `nilai` ON `mahasiswa`.`id` = `nilai`.`id_mahasiswa`
	INNER JOIN `matakuliah` ON `matakuliah`.`id` = `nilai`.`id_matkul`";
...
<a href="<?= $WEB_CONFIG['base_url'] ?>index.php?page=view" class="btn btn-primary mb-3">
	<svg style="width:20px;height:20px" viewBox="0 0 24 24" class="mb-1 m">
	    <path fill="#fff" d="M17,13H13V17H11V13H7V11H11V7H13V11H17M12,2A10,10 0 0,0 2,12A10,10 0 0,0 12,22A10,10 0 0,0 22,12A10,10 0 0,0 12,2Z" />
	</svg> View 
</a>
